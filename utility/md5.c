...
static MD5state*
_md5(byte *p, uint len, byte *digest, MD5state *s)
{
	uint a, b, c, d, tmp;
	uint i, done;
	Table *t;
...
void 
md5(unsigned char *p, unsigned len, unsigned char *digest) {
    ...
    _md5((byte*)copy_p, len, (byte*)digest, &md5_state);
}
